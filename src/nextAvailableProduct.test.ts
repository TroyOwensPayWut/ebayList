...
const rows = [
  { sku: "AB100", enabled: true, hasError: false },
  { sku: "AB101", enabled: false, hasError: false }, // first available
  { sku: "AB102", enabled: false, hasError: true }, // available flag but errored -> skip
  { sku: "AB103", enabled: false, hasError: false }, // next available after AB101
]

// 1. no startSku -> first disabled/no-error row
assert.deepEqual(pickNextAvailableSku(rows), { ok: true, sku: "AB101" })

// 2. startSku matches -> first disabled/no-error row *after* it (skips the start row and errored row)
assert.deepEqual(pickNextAvailableSku(rows, "AB101"), { ok: true, sku: "AB103" })
